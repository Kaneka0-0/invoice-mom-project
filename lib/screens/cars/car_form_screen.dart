...
class _CarFormScreenState extends State<CarFormScreen> {
  ...
  void _load() {
    ...
    final car = context.read<AppProvider>().store.cars.firstWhere(
        (c) => c.id == widget.id,
        orElse: () => Car(id: '', plateNumber: ''));
    ...
    return Consumer<AppProvider>(
      builder: (context, provider, _) {
        ...
        return Scaffold(
          appBar: AppBar(
            title: Text(
                isEdit ? '${s.edit} ${s.cars}' : '${s.add} ${s.cars}'),
            leading: IconButton(
  ...
  Future<void> _save(BuildContext context, AppProvider provider) async {
    ...
    try {
      ...
      if (widget.id == null) {
        ...
        final car =
            provider.store.cars.firstWhere((c) => c.id == widget.id);
